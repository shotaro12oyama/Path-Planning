    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
		  ...
		  // TODO: Sensor fusion to detect other_cars
		  
		  vector<Othercar> other_cars = detect_other_cars(car_s, car_d, sensor_fusion);
		  
		  // Generate trajectory candidate and calcurate cost
		  ...
		  // check & use previous path
          int prev_size = previous_path_x.size();
		  // if previous path size is not enough, start from current
		  if (prev_size < 2) {
		  ...
		  // select the successor lane
		  int lane = select_next_lane(car_d, ref_vel, other_cars);
		  // make the waypoints on the successor lane
		  ...
		  // adjust accelaration
		  double ref_vel = adjust_ref_vel(lane, ref_vel, car_speed, other_cars);
